...
def apply_leave():
    if request.method == 'POST':
        ...
        # Determine recipient based on user's role
        recipient_emails = ROLES.get(current_user.role)
        if not recipient_emails:
            flash("No recipient found for the user's role.", 'error')
            return redirect(url_for('views.apply_leave'))
        ...
        msg.body = f'{current_user.first_name} has applied for a leave.\n\nType: {ltype}\nStart Date: {start_date}\nEnd Date: {end_date}\nDays: {days}\nReason: {reason}'
