...
def forgot_password():
    if request.method == 'POST':
        ...
        if user:
            ...
            reset_token = str(uuid.uuid4())
            user.reset_token = reset_token
            user.reset_token_expiry = datetime.now(india_tz) + timedelta(hours=1)  # Token expires in 1 hour
            db.session.commit()
...
def reset_password(token):
    ...
    if not user or (user.reset_token_expiry and user.reset_token_expiry < datetime.now(india_tz)):
        flash('Invalid or expired token.', 'error')
